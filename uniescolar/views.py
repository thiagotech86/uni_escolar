...
def home(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # Opcional: Armazena o "tipo de visualização" na sessão.
            # As permissões REAIS (ex: para cadastrar aula) devem ser baseadas no perfil
            # real do usuário (superuser, Professor) e não apenas nesta seleção.
    ...
    # Lógica para requisições GET (executada quando a página é carregada ou após um redirect)
    ...
    # A variável 'can_add_aula_permission' para a navbar virá do context_processor que você criou.
    # Se não criou o context_processor, você precisaria adicionar a lógica para 'can_add_aula' aqui:
    # can_add_aula = False
    # if request.user.is_authenticated:
    #     is_professor = Professor.objects.filter(pk=request.user.pk).exists()
    #     if request.user.is_superuser or is_professor:
    #         can_add_aula = True
    # context['can_add_aula'] = can_add_aula
